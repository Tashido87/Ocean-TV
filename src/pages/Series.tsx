...
import { SlidersHorizontal, Search, RotateCcw, Tv } from 'lucide-react';
...
export default function SeriesPage() {
  ...
  const genresList = useMemo(() => {
    ...
    series.forEach(s => s.genres?.forEach(g => list.add(g)));
  ...
  const yearsList = useMemo(() => {
    const list = new Set<string>();
    series.forEach(s => {
      if (s.releaseYear) list.add(s.releaseYear);
    });
  ...
  const networksList = useMemo(() => {
    const list = new Set<string>();
    series.forEach(s => {
      if (s.network) list.add(s.network);
    });
    return ['All', ...Array.from(list)];
  }, [series]);
  ...
  const filteredAndSortedSeries = useMemo(() => {
    ...
    // Year filter
    if (selectedYear !== 'All') {
      result = result.filter(s => s.releaseYear === selectedYear);
    }
    ...
    if (selectedNetwork !== 'All') {
      result = result.filter(s => s.network === selectedNetwork);
    }
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-apple-gray-300 font-medium">
                Browse seasons, detailed episode lists, and networks with dual sub availability
              </p>
            </div>
          </div>

          <div className="text-sm font-semibold text-white/50 bg-white/5 px-4 py-2 rounded-full border border-white/5 self-start">
            Total Series: <span className="text-white">{filteredAndSortedSeries.length}</span>
          </div>
        </div>
        ...
        <div className="glass-panel p-5 sm:p-6 rounded-3xl border border-white/8 flex flex-col gap-6 shadow-2xl">
          <div className="flex flex-col lg:flex-row gap-4 justify-between items-stretch">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search TV series by title, network, or story..."
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 pt-2 border-t border-white/5">
            ...
            {/* Network */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-black text-apple-gray-300 tracking-wider uppercase">Network</label>
              <select
        ...
        {filteredAndSortedSeries.length > 0 ? (
          ...
          <div className="w-full flex flex-col items-center justify-center py-24 text-center glass-panel rounded-3xl border border-white/5">
            ...
            <h3 className="text-lg font-bold text-white mb-1">No series match your filters</h3>
